...
contract CoinFlip is VRFConsumerBaseV2Plus {
    ...
    uint256[2] private  COIN_NUMBER = [0, 1];
   ...
   IERC20 public betToken;
    ...
    event RouletteSpined(uint256 indexed  requestId, address indexed roller);
    event RouletteLanded(uint256 indexed  requestId, uint256 indexed result);

    event BetPlaced(address indexed player, uint256 amount, uint256 number);
    event CoinFliped(uint256 indexed requestId, address indexed player);
    ...
    function placeBet(uint256 _number, uint256 _amount) external {
        require(_number >= 0 && _number <= 1, "Invalid number");
        ...
        emit CoinFliped(requestId, msg.sender);
    }
    ...
    function fulfillRandomWords(
        ...
        uint256 luckyNumber = COIN_NUMBER[randomWords[0] % 2];
